...
public class ExceptionRatioCircuitBreakerDemo {
        ...
        EventObserverRegistry.getInstance().addStateChangeObserver("logging",
                (prevState, newState, rule, snapshotValue) -> {
                    if (newState == State.OPEN) {
                        System.err.println(String.format("%s 状态修改为 OPEN 时间戳: %d, snapshotValue=%.2f", prevState.name(),
                                TimeUtil.currentTimeMillis(), snapshotValue));
                        System.err.println("snapshotValue=" + snapshotValue);
                    } else {
                        System.err.println(String.format("%s 状态修改为 %s 时间戳: %d", prevState.name(), newState.name(),
    ...
    static class TimerTask implements Runnable {
        ...
        public void run() {
            ...
            System.out.println("任务开始! Begin to run! Go go go!");
